<template>
  <div class="container">
    ...
    <div class="jumbotron">
      <h1>HELLO SHOP_ADMIN</h1>
      <p class="lead">관리자 회원 기능</p>
        ...
        <v-layout align-center>
          ...
          <v-flex xs12 sm8 md4>
              ...
              <v-card-actions>
                ...
                <v-btn depressed large color="blue white--text" @click="moveMemberList">회원 목록</v-btn>
              </v-card-actions>

            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn depressed large color="green white--text" @click="moveCreateItemForm">상품 등록</v-btn>
...
<!--        <a class="btn btn-lg btn-secondary" v-on:click="moveMemberList">회원 목록</a>-->
<!--      </p>-->
<!--      <p class="lead">상품 기능</p>-->
<!--      <p>-->
<!--        <a class="btn btn-lg btn-dark"  v-on:click="moveCreateItemForm">상품 등록</a>-->
...
export default {
  ...
  created() {
    //this.PK_id = this.$route.query.id;
  ...
  methods :{
    moveUpdateFormMember() {
      ...
      this.$axios.get('http://localhost:8080/members/' + this.PK_id + '/MyUpdate'
          ,params)
          .then((res) => {
    ...
    moveCreateItemForm() {
      this.$router.push({name: 'createItemForm'});
    }
    ,
    moveItemList() {
    ...
    moveOrderList() {
      this.$axios.get('http://localhost:8080/ordersInit')
          .then((res) => {
            this.resultData = res.data;
            console.log(this.resultData);
            this.$router.push({name: 'orderList', query: {orderstateList: this.resultData}});
          }).catch((err)=>{
            console.log(err);
          })
